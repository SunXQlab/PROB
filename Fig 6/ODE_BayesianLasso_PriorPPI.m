function [Para_Post_pdf,S]=ODE_BayesianLasso_PriorPPI(Data_smooth,DPP,PriorPPI)
...
% if nargin>2
    x_input(:,PriorPPI(ind_gene,:)==0)=[]; % exclude genes not regulating this gene (priorPPI) 
% end
% x_input(:,ind_gene)=[];  % Because the above line has riled to exclude 'ind_gene'
% [A,FitInfo] = lasso(x_input,y_output,'Alpha',1,'CV',10,'Standardize',1,'Intercept_Flag',0);  % modified from matlab function by adding Intercept_Flag argument associated with lassoFit_Sun (redefined function)
...
%     coef(ind_gene)=0;
    [EstMdl,Summary] = estimate(PriorMdl,X_train,Y_train,'Display',false);  %,'Sampler',"hmc",'NumDraws',1e6,'Burnin',1e3,'BetaStart',[coef]
...
Summary = Summary.MarginalDistributions;
    ...
    Summary_new(find(PriorPPI(ind_gene,:)==1),:)=[Summary.Mean(1:end-2),Summary.Std(1:end-2)];
    Mean=Summary_new(:,1);
    Std=Summary_new(:,2);
...
%     MUCI = CI_Sun(Para,k_alpha);  % 
    MUCI = CI_Sun_Normal_PriorPPI(Summary,k_alpha);  % 
