...
for rep_root=1:size(data,1)   %randperm(size(data,1))
    a=sum(grade==1|grade==2|grade==3)+1;
    b=length(grade);
    rn=ceil(rand*(b-a)+a);
...
%     AA_sort
%     grade(AA_sort)
%     AA=find(grade(AA_sort)==min(grade))
%     root=AA(1);
...
% 
% %%%% Real time vs DPT
[RHO,PVAL]=corr(X_stage(8,:)',DPT,'type','Spearman')
figure,
% plot(X_stage(8,:),DPT,'-','color',[0.5 0.5 0.5],'LineWidth',2)
% hold on
scatter(X_stage(8,:)/max(X_stage(8,:)),DPT/max(DPT),40,DPT/max(DPT),'fill')
set(gca,'FontSize',15)
...
% %  Hirerachical clustering vs Real progression
% figure,
% Z=linkage(pdist(data));
% % scatter(X_stage(8,:),[Z(:,1)',100],50,[Z(:,1)',100]/100,'fill')
% % set(gca,'FontSize',15)
% % xlabel('Real progression','FontSize',20);
% % ylabel('Diffusion pseudo-progression (Norm.)','FontSize',15)
% % colorbar
% 
% T=cluster(Z2,6);
% H=dendrogram(Z2);
% T1=clusterdata(data,0.00001);
% figure,scatter(X_stage(8,:),100-T1,50, (100-T1),'fill')   %(100-T1)/max(100-T1)
% set(gca,'FontSize',15)
% xlabel('Real progression','FontSize',20);
% ylabel('Hirerachical clustering ordering','FontSize',15)
% colorbar
% 
% 
% figure,
% scatter(100-T1,data(:,g)',50,'fill')
% hold on
% plot(100-T1,data(:,g)','-','color',[0.5 0.5 0.5],'LineWidth',1)  %  
% set(gca,'FontSize',15)
% xlabel('time','FontSize',20);
% ylabel('Gene expression','FontSize',20)
% 
% cgo=clustergram(data);
% set(cgo,'Rowlabels',row(1:100),'Column',column(1:6))
